...
void testReadWrite()
{
   ...
   for (int i = 0; i < numberOfTrials; i++)
   {
      ...
      if ((errorCode = writeDisk(lba, writeBuffer)) != SIM_DEV_SUCCESS)
      {
      ...
      if ((errorCode = readDisk(lba, size, &readBuffer)) != SIM_DEV_SUCCESS)
      {
...
void testWithTime(int sig) {
    ...
    switch (rand() % 2) {
        case 0:
            if ((errorCode = readDisk(lba, size, (void *) &readBuffer)) == SIM_DEV_SUCCESS) {
                printf("READ %d bytes:\n%s\n", size, readBuffer);
                free(readBuffer);
            } else
                printf("\n*** ERROR READING %d BYTES: %d\n\n", size, errorCode);
