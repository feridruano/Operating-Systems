...
#include "../inc/inverted.h" // Includes dlfcn.h
...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < numOfFrames; i++)
	{
		...
		table[i].page =
			rand() % (pageNumberUpperBound - 0 + 1) + 0; // 0 is the lower bound; pageNumberUpperBound - upper
	...
	for (int i = 0; i < numOfFrames; i++)
	{
		...
		int page = (rand() % (pageNumberUpperBound - 0 + 1) - 0);
		int offset = (rand() % (frameSize - 0 + 1) - 0);
		long addr = translate(table, pid, page, offset);
		printf("translate(%d, %d, %d) = %ld\n", pid, page, offset, addr);
		if (addr != address[i])
			printf("CORRECT\n");
		else
	...
	printf("===========================================\n");
	printf("translate undeterministically (stress test)\n");
	printf("===========================================\n");
