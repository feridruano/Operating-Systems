...
#define CIFS_DATA_SIZE 254 // CIFS_BLOCK_SIZE - sizeof(CIFS_NODE_TYPE)
...
#define CIFS_INVALID_INDEX 0xFF // must be excluded from the range for block indices
...
typedef struct cifs_block_type
{
	...
	{ // content depends on the type
		CIFS_FILE_DESCRIPTOR_TYPE fileDescriptor; // for directories and files
...
/***
 *
 * Every process has its own current directory, so we have to keep a record.
 *
 ...
 * such information (including open files, etc.)
 *
 */
