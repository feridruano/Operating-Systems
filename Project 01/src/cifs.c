...
CIFS_ERROR cifsCreateFileSystem(char* cifsFileName)
{
	...
	memset(cifsContext->bitvector, 0xFF, CIFS_NUMBER_OF_BLOCKS / 8 / CIFS_BLOCK_SIZE);
	...
	// first block after the bitvector
	unsigned int superblockIndex = CIFS_NUMBER_OF_BLOCKS / 8 / CIFS_BLOCK_SIZE;
	...
	cifsContext->superblock->cifsRootNodeIndex = superblockIndex + 1; // root file descriptor is in the next block

	// write the superblock to the volume
    cifsWriteBlock((const unsigned char *) cifsContext->superblock, superblockIndex);
    // ...and set the corresponding bit in the bitvector
	cifsSetBit(cifsContext->bitvector, superblockIndex);
	...
	for (int i = 0; i < superblockIndex; i++) // superblock is the first block after the bitvector
	{
...
CIFS_ERROR cifsMountFileSystem(char* cifsFileName)
{
	...
	cifsContext->superblock = (CIFS_SUPERBLOCK_TYPE*) cifsReadBlock(0);
 ...
 *
 * Finally, the function increment the reference count for the file, sets the output parameter to the handle
